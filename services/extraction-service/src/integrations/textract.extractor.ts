...
export async function extractWithTextract(input: {
  ...
  const isPdf = input.contentType.toLowerCase() === "application/pdf";

  // AnalyzeExpense is synchronous and only supports single images (JPEG/PNG/TIFF).
  // Multi-page PDFs — bank statements AND receipt PDFs — require the async
  // StartDocumentAnalysis API or they throw UnsupportedDocumentException.
  if (input.source === "bank_statement" || isPdf) {
...
// ── Bank Statement — StartDocumentAnalysis (async, supports multi-page PDFs) ──
...
async function extractBankStatement(input: {
  ...
  const startRes = await input.textract.send(new StartDocumentAnalysisCommand({
    ...
    FeatureTypes: ["TABLES"],
    OutputConfig: { S3Bucket: input.bucket, S3Prefix: "textract-output" }
  }));
  ...
  for (let attempt = 0; attempt < 30; attempt++) {
    ...
    const getRes = await input.textract.send(new GetDocumentAnalysisCommand({ JobId: jobId }));
    ...
    if (getRes.JobStatus === "SUCCEEDED") {
      ...
      while (pageToken) {
        const pageRes = await input.textract.send(new GetDocumentAnalysisCommand({ JobId: jobId, NextToken: pageToken }));
...
// ── Receipt — AnalyzeExpense (synchronous) ────────────────────────────────────
...
async function extractReceipt(input: {
  ...
  const res = await input.textract.send(new AnalyzeExpenseCommand({
  ...
  for (const group of doc.SummaryFields ?? []) {
    ...
    const confidence = group.ValueDetection?.Confidence ? group.ValueDetection.Confidence / 100 : undefined;
  ...
  for (const group of doc.LineItemGroups ?? []) {
    for (const row of group.LineItems ?? []) {
      ...
      for (const field of row.LineItemExpenseFields ?? []) {
        ...
        const conf = field.ValueDetection?.Confidence ? field.ValueDetection.Confidence / 100 : 1;
      ...
      if (item.description || item.totalPrice) lineItems.push(item);
    }
  }
...
function parseBankStatementTables(blocks: Block[]): ExtractedLineItem[] {
  ...
  for (const table of tables) {
    ...
    allItems.push(...parseTransactionRows(rows, statementYear));
  }
...
function detectStatementYear(blocks: Block[]): number {
  ...
  for (const block of blocks) {
    ...
    const yearMatch = (block.Text ?? "").match(/\b(20\d{2})\b/);
    if (yearMatch) {
      const year = parseInt(yearMatch[1]);
      if (Math.abs(year - currentYear) <= 2) return year;
    }
  }
  return currentYear;
}
...
function extractTableRows(table: Block, blockMap: Map<string, Block>): string[][] {
  ...
  for (const rel of table.Relationships ?? []) {
    ...
    for (const cellId of rel.Ids ?? []) {
      ...
      const key = `${cell.RowIndex ?? 0}-${cell.ColumnIndex ?? 0}`;
      let text = "";
      ...
      for (const cellRel of cell.Relationships ?? []) {
        ...
        for (const wordId of cellRel.Ids ?? []) {
          ...
          if (word?.BlockType === "WORD") text += (text ? " " : "") + (word.Text ?? "");
        }
      }
  ...
  for (let r = 1; r <= maxRow; r++) {
    const row: string[] = [];
    for (let c = 1; c <= maxCol; c++) row.push(cellMap.get(`${r}-${c}`) ?? "");
    rows.push(row);
  }
...
function parseTransactionRows(rows: string[][], statementYear: number): ExtractedLineItem[] {
  ...
  for (let i = 0; i < header.length; i++) {
    const h = header[i];
    if (dateCol   === -1 && (h.includes("date") || h.includes("transaction"))) dateCol = i;
    if (descCol   === -1 && (h.includes("description") || h.includes("merchant") || h.includes("memo"))) descCol = i;
    if (amountCol === -1 && h.includes("amount")) amountCol = i;
  }

  // Fallback: positional assignment for 3-column tables
  let usingPositionalFallback = false;
  if (dateCol === -1 && descCol === -1 && amountCol === -1 && rows[0].length >= 3) {
    dateCol   = 0;
    descCol   = rows[0].length === 3 ? 1 : 2;
    amountCol = rows[0].length - 1;
    usingPositionalFallback = true;
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (description?.toLowerCase().includes("description") || description?.toLowerCase().includes("total")) continue;

    const amount = parseTransactionAmount(amountRaw);
    if (amount === undefined || amount <= 0) continue;

    // For positional-fallback tables (no header match), require a parseable date in col 0.
    // This prevents non-transaction tables (e.g. payment warning tables) from producing rows.
    const date = parseDateString(dateRaw, statementYear);
    if (usingPositionalFallback && !date) continue;
...
function parseTransactionAmount(raw: string | undefined): number | undefined {
...
function parseDateString(raw: string | undefined, statementYear: number): string | undefined {
  if (!raw) return undefined;
  ...
  if (md) {
    const [, m, d] = md;
    return `${statementYear}-${m.padStart(2, "0")}-${d.padStart(2, "0")}`;
  }
