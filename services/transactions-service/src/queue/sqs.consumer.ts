// File: services/transactions-service/src/queue/sqs.consumer.ts
...
export class SqsConsumerService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  constructor(private readonly transactionsService: TransactionsService) {
    ...
    this.queueUrl = mustGetEnv("SQS_TRANSACTIONS_QUEUE_URL");
  }
  ...
  private async handleMessage(body: string | undefined, receiptHandle: string | undefined): Promise<void> {
    ...
    let event: ExtractionCompletedEvent | null = null;

    try {
      // Unwrap SNS envelope
      const outer = JSON.parse(body) as { Message?: string };
      const inner = outer.Message ? JSON.parse(outer.Message) : JSON.parse(body);
      event = inner as ExtractionCompletedEvent;

      if (event.type !== "extraction.completed.v1") {
        this.log.warn({ eventType: event.type }, "Skipping unknown event type");
        await this.deleteMessage(receiptHandle);
        return;
      }

      await this.transactionsService.handleExtractionCompleted(event);
      await this.deleteMessage(receiptHandle);

      this.log.info(
        { uploadFileId: event.uploadFileId, extractedDocumentId: event.extractedDocumentId },
        "Message processed and deleted"
      );
    } catch (err) {
      this.log.error({ err, uploadFileId: event?.uploadFileId }, "Failed to process message");
